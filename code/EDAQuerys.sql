...
SELECT area, COUNT(area) AS 'total_accidente' FROM  accidente GROUP BY Area

--Pregunta 2: ¿Qué día de la semana tiene el mayor número de accidentes?
SELECT * FROM accidente
SELECT TOP 3 dia, COUNT(Dia) AS 'total' FROM accidente GROUP BY Dia ORDER BY total DESC
...
SELECT EdadGrupo, COUNT(EdadGrupo) AS EdadCount, AVG(EdadVehiculo) AS PromedioEdad
FROM (
    SELECT EdadVehiculo,
           CASE
           ...
           END AS EdadGrupo
    FROM vehiculo
    WHERE EdadVehiculo IS NOT NULL
) AS SubQuery
GROUP BY EdadGrupo;
...
--Pregunta 6:  ¿Cuántos accidentes ocurrieron para cada tipo de dirección de volante?

SELECT * FROM vehiculo

SELECT ManoIzquierda, COUNT(ManoIzquierda) AS Accidentes_Totales
FROM vehiculo
GROUP BY ManoIzquierda
ORDER BY 2 DESC
...
FROM accidente
GROUP BY Gravedad, Condiciones_del_camino
ORDER BY Total DESC
...
FROM vehiculo v JOIN accidente a ON v.AccidenteIndex = a.AccidenteIndex
GROUP BY v.PropositoViaje, a.Gravedad
ORDER BY 1 DESC;